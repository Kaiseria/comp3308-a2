...
\author{Student 1: \textless SID: 540314324\textgreater \\ Student 2: \textless SID: 540108295\textgreater}
\date{}
...
MyKNN+ was developed to improve the original MyKNN algorithm by changing the voting method. In MyKNN, the distance between a testing instance and each training instance was calculated by counting how many attribute values were different. After sorting the training instances by distance, the k nearest neighbours were selected. However, each selected neighbour had the same voting power, regardless of whether it was very close or relatively far from the testing instance. This could be a weakness because a patient that is almost identical to the testing patient should logically provide stronger evidence than a patient that only happens to be included among the nearest neighbours.

For this reason, we changed MyKNN into MyKNN+ by using distance-weighted voting. We kept the same distance calculation so that the comparison between MyKNN and MyKNN+ would be fair, but changed the voting score so that closer neighbours had more influence. The weight was calculated as 1 / (distance + 0.25). The value 0.25 was added to avoid division by zero when the distance was 0, while still giving an exact or very close match a strong vote. For example, a neighbour with distance 0 receives a weight of 4, while a neighbour with distance 3 receives only about 0.31. This means the prediction is influenced more by patients with very similar medical attributes.

This modification was motivated by the nature of the heart failure dataset. Since each instance represents a patient, it is reasonable to assume that patients with more similar attribute values may have more similar outcomes. If all neighbours vote equally, the prediction may be affected too much by less similar patients, especially when the dataset is small and the selected k neighbours may include some noisy or less relevant cases. Distance-weighted voting was therefore used to reduce the influence of these weaker neighbours and make the prediction more sensitive to the closest matches.

The results showed that this modification improved the model. MyKNN achieved an accuracy of 63.37\%, while MyKNN+ improved to 67.40\%. This increase suggests that the original equal-voting method was not using the distance information effectively after selecting the nearest neighbours. By giving more importance to closer neighbours, MyKNN+ was able to make better predictions overall. However, the improvement was moderate rather than very large, which indicates that distance weighting helped but did not fully solve the limitations of KNN on this dataset.

One possible reason is that many attributes in the dataset are categorical or discretised into groups, so the distance measure only counts whether values are the same or different. This simple distance measure may not fully capture medical similarity between patients. Also, because the dataset is small and moderately imbalanced, KNN may still struggle to identify the minority died class. Therefore, the result indicates that distance-weighted voting was a useful improvement, but KNN was still less suitable for this dataset than stronger models such as SMO, OneR, and the pruned decision tree.
...
For the KNN classifiers, Weka KNN performed better than both MyKNN and MyKNN+. Weka KNN achieved an accuracy of 70.70\%, while MyKNN achieved 63.37\% and MyKNN+ achieved 67.40\%. This shows that our distance-weighted modification improved the original MyKNN, but it was still not as strong as Weka's implementation.

One reason for this difference is that Weka KNN is a more developed implementation and may handle distance calculation, normalisation, tie-breaking, and parameter settings more effectively. In comparison, our MyKNN used a simple distance method that counted how many attribute values were different. This method is easy to understand, but it may not fully capture the medical similarity between patients, especially when the dataset contains discretised or categorical attributes.

The comparison indicates that MyKNN+ was a useful improvement over MyKNN, because giving closer neighbours more influence increased the accuracy. However, the lower result compared with Weka KNN suggests that the performance of KNN depends strongly on the distance function and implementation details. Therefore, while our KNN+ modification improved the model, there is still room for further improvement, such as testing different values of k, using a better distance measure, or applying attribute weighting.
...
\textbf{Student 1: \textless SID: 540314524\textgreater}

In this assignment, I learned how to applied the models in the lectures to Python programs and solved some coding programs. This connects my knowledge to real practicing and improved my ability on coding. Moreover, I learned that more complex models are not always better. We should choose the best model based on the dataset and the nature of the data to make prediction.

\textbf{Student 2: \textless SID: 540108295\textgreater}

Write your reflection here.

\section{Acknowledgement of AI Use}

\textbf{Student 1: \textless SID: 540314324\textgreater}

I have used ChatGPT-5.5 for the assignment. I used AI to:

- Fixing the errors in my KNN+ and DT+ pragrams

- Helping to generate the LaTeX file

- Normalising Python program comments

\textbf{Student 2: \textless SID: 540108295\textgreater}


I used ChatGPT-5.5 to help improve the wording, grammar and structure of parts of the report.
I also used it to help explain some machine learning concepts, such as cross-validation, pruning, precision, recall and F1-measure. Also help guiding with suggestions and method through testing 
All generated content was reviewed and edited before being included in the final submission.
